...
namespace Settlement
{
    class WorldGenerator
    {
        ...
        {
            int seed = 123; // Change to whatever
            ...
            double frequency = 0.023; // Adjust for mountains/hills/plains (default 0.01)
        ...
        {
            // Takes in the blockPos, which is relative to world space
            // Returns what category block it should be
            byte cat;
            double posHeight = perlinNoise.Get3D(blockPos.X, blockPos.Y, blockPos.Z);
            //double posHeight = perlinNoise.Get2D(blockPos.X, blockPos.Z);
            //double posHeight = perlinNoise.Get2D((int)blockPos.X, (int)blockPos.Z);
            if (blockPos.Y < posHeight)
                cat = 1; // Stone
            else
                cat = 0; // Air
            return cat;
